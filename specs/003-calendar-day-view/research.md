...
- Escape-to-close and focus-return-to-trigger are automatic/built-in (documented keyboard table: "Esc — Closes the dialog and moves focus to `DialogTrigger`") — this satisfies FR-017 without extra code.
...
- Exact additional emit names (e.g., a possible `escapeKeyDown` or `interactOutside` emit distinct from `pointerDownOutside`) should be confirmed against the installed version's type definitions during implementation; the `pointer-down-outside` event alone is sufficient to distinguish "closed via outside click" from "closed via Escape" for FR-024, since Escape does not fire it.
