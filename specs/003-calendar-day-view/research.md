...
**Pattern worth carrying into 004 (drag-and-drop)**: distinguish *pointer-triggered* focus from *keyboard-triggered* focus before running any scroll/viewport side effect on a `focus` handler — a pointer interaction (click, and presumably drag) can move focus to a container as a native side effect completely unrelated to the interaction's own intent, and any `focus`-driven scroll-into-view or auto-centering logic needs the `:focus-visible` guard (or equivalent) to avoid fighting the pointer interaction that's still in flight.
