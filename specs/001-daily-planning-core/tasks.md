...
`backend/{domain,application,infrastructure,bootstrap}/src/{main,test}/java/dev/puzzleagenda/...`
...
- [ ] T001 Create the backend Maven multi-module skeleton (parent `backend/pom.xml` plus
...
- [ ] T002 Initialize the frontend Vite + Vue 3 (Composition API) project, no state-management
      library, in `frontend/` (plan.md Technical Context; Constitution Principle V)
- [ ] T003 [P] Add backend test dependencies (JUnit 5, AssertJ, Mockito, Spring Boot Test,
      Testcontainers PostgreSQL module) to the relevant module `pom.xml` files under `backend/`
- [ ] T004 [P] Add frontend test tooling (Vitest, Vue Test Utils) to `frontend/package.json`
- [ ] T005 [P] Configure the Flyway Maven plugin and PostgreSQL 16+ dev connection properties in
...
- [ ] T006 Create Flyway migration for the `activity` and `time_block` tables, including the
...
- [ ] T007 [P] Create Flyway migration for the `routine_template_entry` table in
...
- [ ] T008 [P] Create Flyway migration for the `materialized_day` and `horizon_state` tables in
...
- [ ] T009 Implement the `TimeRange` value object enforcing 5-minute granularity and half-open
      `[start, end)` semantics in
      `backend/domain/src/main/java/dev/puzzleagenda/domain/timeblock/TimeRange.java`
      (research.md §1, §6)
- [ ] T010 [P] Implement the `BlockType` enum (`ROUTINE`, `CONSTRAINED`, `PLANNED_ACTIVITY`) in
      `backend/domain/src/main/java/dev/puzzleagenda/domain/timeblock/BlockType.java`
- [ ] T011 [P] Implement the `Priority` enum (`LOW`, `MEDIUM`, `HIGH`) in
      `backend/domain/src/main/java/dev/puzzleagenda/domain/activity/Priority.java`
- [ ] T012 Implement the `HorizonState` entity and Day-1/forward-bound reachability rules in
      `backend/domain/src/main/java/dev/puzzleagenda/domain/horizon/HorizonState.java`
      (research.md §5; data-model.md HorizonState)
- [ ] T013 [P] Implement the `MaterializedDay` marker entity in
      `backend/domain/src/main/java/dev/puzzleagenda/domain/horizon/MaterializedDay.java`
      (research.md §4)
- [ ] T014 Define repository ports `ActivityRepository`, `TimeBlockRepository`,
      `RoutineTemplateRepository`, `HorizonStateRepository`, `MaterializedDayRepository` in
      `backend/domain/src/main/java/dev/puzzleagenda/domain/port/` (plan.md Project Structure)
- [ ] T015 [P] Parameterized domain unit tests for `HorizonState` reachability (`day1` null vs.
      set, forward bound = `today + 13`, before/after both bounds) in
      `backend/domain/src/test/java/dev/puzzleagenda/domain/horizon/HorizonStateTest.java`,
      plain JUnit 5 + AssertJ (Constitution Principle III; spec Edge Cases — day before Day 1,
      day 13 vs. day 14)
- [ ] T016 Implement a global REST exception handler mapping domain/application exceptions to
      `{ "reason", "message" }` bodies with the 400/404/409/422 codes from contracts/api.md's
      Error Conventions table, in
      `backend/infrastructure/src/main/java/dev/puzzleagenda/infrastructure/rest/ApiExceptionHandler.java`
- [ ] T017 Wire the Spring Boot bootstrap module (main class, module component scanning,
      datasource + Flyway config) in
      `backend/bootstrap/src/main/java/dev/puzzleagenda/bootstrap/PuzzleAgendaApplication.java`
      and `backend/bootstrap/src/main/resources/application.yml`
- [ ] T018 [P] Implement `GetHorizon` use case and `GET /api/horizon` endpoint returning
      `{ day1, forwardBound }` in
      `backend/application/src/main/java/dev/puzzleagenda/application/day/GetHorizon.java` and
      `backend/infrastructure/src/main/java/dev/puzzleagenda/infrastructure/rest/DayController.java`
      (contracts/api.md `GET /api/horizon`)
- [ ] T019 [P] Implement the base frontend HTTP client wrapper in `frontend/src/api/client.js`
...
- [ ] T020 [P] [US1] Parameterized domain unit tests for `OverlapPolicy` (adjacent-allowed,
      ...
      `backend/domain/src/test/java/dev/puzzleagenda/domain/service/OverlapPolicyTest.java`
...
- [ ] T021 [P] [US1] Application unit tests (mocked repositories) for `CreateTimeBlock`,
      ...
      `backend/application/src/test/java/dev/puzzleagenda/application/timeblock/TimeBlockUseCasesTest.java`
- [ ] T022 [P] [US1] Infrastructure contract tests for `POST /api/days/{date}/blocks`,
      ...
      `backend/infrastructure/src/test/java/dev/puzzleagenda/infrastructure/rest/TimeBlockControllerIT.java`
...
- [ ] T024 [US1] Implement the `TimeBlock` entity (start/end via `TimeRange`, derived `day`,
      ...
      `backend/domain/src/main/java/dev/puzzleagenda/domain/timeblock/TimeBlock.java`
      (depends on T009, T010)
- [ ] T025 [US1] Implement the `OverlapPolicy` domain service (half-open interval intersection)
      in `backend/domain/src/main/java/dev/puzzleagenda/domain/service/OverlapPolicy.java`
...
- [ ] T026 [US1] Implement `CreateTimeBlock` (validates horizon reachability via `HorizonState`,
      ...
      `backend/application/src/main/java/dev/puzzleagenda/application/timeblock/CreateTimeBlock.java`
      (depends on T012, T025; research.md §5)
- [ ] T027 [US1] Implement `EditTimeBlock` (start/end/name edit, same day, overlap re-check) in
      `backend/application/src/main/java/dev/puzzleagenda/application/timeblock/EditTimeBlock.java`
      (depends on T025)
- [ ] T028 [US1] Implement `DeleteTimeBlock` (`ROUTINE`/`CONSTRAINED`: plain removal) in
      `backend/application/src/main/java/dev/puzzleagenda/application/timeblock/DeleteTimeBlock.java`
      (depends on T024)
- [ ] T029 [US1] Implement `ViewDay` returning a day's blocks in chronological order (US4 wires
      ...
      `backend/application/src/main/java/dev/puzzleagenda/application/day/ViewDay.java`
      (depends on T012, T013)
- [ ] T030 [US1] Implement the `TimeBlockRepository` persistence adapter, including mapping
      to/from the `span tsrange` column, in
      `backend/infrastructure/src/main/java/dev/puzzleagenda/infrastructure/persistence/TimeBlockRepositoryAdapter.java`
...
- [ ] T032 [US1] Implement `TimeBlockController` (`POST /api/days/{date}/blocks`,
      ...
      `backend/infrastructure/src/main/java/dev/puzzleagenda/infrastructure/rest/TimeBlockController.java`
...
- [ ] T037 [P] [US2] Application unit tests for `CreateActivity`/`EditActivity`/`DeleteActivity`
      in `backend/application/src/test/java/dev/puzzleagenda/application/activity/ActivityUseCasesTest.java`
- [ ] T038 [P] [US2] Infrastructure contract tests for `/api/activities` endpoints
      (Spring Boot Test + Testcontainers) in
      `backend/infrastructure/src/test/java/dev/puzzleagenda/infrastructure/rest/ActivityControllerIT.java`
...
- [ ] T040 [P] [US2] Implement the `Activity` entity (name, duration, priority, category;
      ...
      `backend/domain/src/main/java/dev/puzzleagenda/domain/activity/Activity.java`
      (depends on T011)
- [ ] T041 [US2] Implement `CreateActivity`, `EditActivity`, `DeleteActivity` (direct delete
      only while `UNPLANNED`, FR-004) in
      `backend/application/src/main/java/dev/puzzleagenda/application/activity/CreateActivity.java`,
      `.../EditActivity.java`, `.../DeleteActivity.java` (depends on T040)
- [ ] T042 [US2] Implement the `ActivityRepository` persistence adapter in
      `backend/infrastructure/src/main/java/dev/puzzleagenda/infrastructure/persistence/ActivityRepositoryAdapter.java`
...
- [ ] T043 [US2] Implement `ActivityController` (`GET`/`POST`/`PUT /api/activities`,
      ...
      `backend/infrastructure/src/main/java/dev/puzzleagenda/infrastructure/rest/ActivityController.java`
      (depends on T041)
...
- [ ] T047 [P] [US3] Domain unit tests for `Activity` `UNPLANNED`/`PLANNED` transitions in
      `backend/domain/src/test/java/dev/puzzleagenda/domain/activity/ActivityTest.java`
- [ ] T048 [P] [US3] Application unit tests for planning/moving/deleting a `PLANNED_ACTIVITY`
      ...
      `backend/application/src/test/java/dev/puzzleagenda/application/timeblock/PlanActivityTest.java`
- [ ] T049 [P] [US3] Infrastructure contract tests for `POST /api/days/{date}/blocks` with
      ...
      `backend/infrastructure/src/test/java/dev/puzzleagenda/infrastructure/rest/PlanActivityControllerIT.java`
...
- [ ] T051 [US3] Implement `MoveTimeBlock` (reschedule a `PLANNED_ACTIVITY` block to a new
      ...
      `backend/application/src/main/java/dev/puzzleagenda/application/timeblock/MoveTimeBlock.java`
...
- [ ] T053 [US3] Extend `DeleteActivity` to require `confirm=true` and cascade-delete the
      ...
      `backend/application/src/main/java/dev/puzzleagenda/application/activity/DeleteActivity.java`
...
- [ ] T057 [P] [US4] Domain unit tests for `RoutineTemplateEntry` overlap validation using the
      ...
      `backend/domain/src/test/java/dev/puzzleagenda/domain/routine/RoutineTemplateEntryTest.java`
      (FR-016)
- [ ] T058 [P] [US4] Parameterized domain unit tests for `MaterializationService` clipping (full
      ...
      `backend/domain/src/test/java/dev/puzzleagenda/domain/service/MaterializationServiceTest.java`
...
- [ ] T059 [P] [US4] Infrastructure contract tests for routine-template CRUD and
      ...
      Testcontainers) in
      `backend/infrastructure/src/test/java/dev/puzzleagenda/infrastructure/rest/RoutineTemplateControllerIT.java`

### Implementation for User Story 4

- [ ] T060 [US4] Implement the `RoutineTemplateEntry` entity with two-day-projection overlap
      validation in
      `backend/domain/src/main/java/dev/puzzleagenda/domain/routine/RoutineTemplateEntry.java`
      (depends on T009)
- [ ] T061 [US4] Implement `MaterializationService` (project each entry onto the target day,
      ...
      sub-interval) in
      `backend/domain/src/main/java/dev/puzzleagenda/domain/service/MaterializationService.java`
      (depends on T024, T060; research.md §3)
- [ ] T062 [US4] Implement `CreateRoutineEntry`, `EditRoutineEntry`, `DeleteRoutineEntry` in
      `backend/application/src/main/java/dev/puzzleagenda/application/routine/CreateRoutineEntry.java`,
...
- [ ] T064 [US4] Implement the `RoutineTemplateRepository` and `MaterializedDayRepository`
      ...
      `backend/infrastructure/src/main/java/dev/puzzleagenda/infrastructure/persistence/RoutineTemplateRepositoryAdapter.java`
...
- [ ] T065 [US4] Implement `RoutineTemplateController`
      ...
      `backend/infrastructure/src/main/java/dev/puzzleagenda/infrastructure/rest/RoutineTemplateController.java`
      (depends on T062)
...
Task: "Parameterized domain unit tests for OverlapPolicy in backend/domain/src/test/java/dev/puzzleagenda/domain/service/OverlapPolicyTest.java"
Task: "Application unit tests for CreateTimeBlock/EditTimeBlock/DeleteTimeBlock in backend/application/src/test/java/dev/puzzleagenda/application/timeblock/TimeBlockUseCasesTest.java"
Task: "Infrastructure contract tests for time-block endpoints in backend/infrastructure/src/test/java/dev/puzzleagenda/infrastructure/rest/TimeBlockControllerIT.java"
